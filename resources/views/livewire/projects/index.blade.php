<div>
    @forelse ($projects as $project)
        <div wire:key='project-{{ $project->id }}' class="bg-white shadow-sm rounded mt-2 p-4 flex flex-col items-center justify-center sm:flex-row gap-5 sm:justify-between">
            ...
            <div class="flex justify-between gap-2">
                ...
                <!-- TOGGLE ACTIVE -->
                <button type="button" wire:click="toggleActive({{$project->id}})" class="p-2 text-xs focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 rounded {{ $project->active ? 'bg-red-100 text-red-500 hover:text-red-600' : 'bg-green-100 text-green-600 hover:text-green-600' }} transition-colors duration-300 ease-linear">
                    {{ $project->active ? 'Desactivar' : 'Activar' }}
                </button>

                <a href="{{ route('technologies.edit', $project) }}"
